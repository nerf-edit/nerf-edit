...
def save_panoptic(masks_probs, confidence, out_filename):
        ...
        torch.save(
            {
                ...
                "probabilities": masks_probs,
                "confidences": confidence,
                "text_feats": predictions["res3_feats"]
            }, fid
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--labels", type=str, required=True, help="all labels as text prompt")
    parser.add_argument(
        "--output", "-o", type=str, default="panoptic", required=True, help="output directory"
    )
    ...
    labels = args.labels
    # convert list of labels to text prompt
    ...
    for path in tqdm.tqdm(used_files, disable=not args.output):
        path = str(path)
        image_pil, image = load_image(path)
        ...
        masks_probs, confidence, _ = predictor.predict_torch(
            point_coords = None,
            point_labels = None,
        ...
        # )
        save_panoptic(masks_probs, confidence, demo, output_dir + ".ptz")
